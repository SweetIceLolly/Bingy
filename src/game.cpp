...
void postSignInCallback(const cq::MessageEvent &ev) {
    try {
        ...
        cq::send_group_message(GROUP_ID, bg_at(ev) +
            ...
            "获得硬币" + std::to_string(deltaCoins) + ", 目前拥有" + std::to_string(PLAYER.get_coins()) +
            "; 获得体力" + std::to_string(deltaEnergy) + ", 目前拥有" + std::to_string(PLAYER.get_energy()) +
            "; 获得经验: " + std::to_string(deltaExp) + (eventMsg.empty() ? "" : eventMsg)
        );
    }
...
bool preFightCallback(const cq::MessageEvent &ev, const std::string &arg, LL &dungeonLevel) {
    ...
    try {
        ...
        if (allDungeons.find(dungeonLevel) == allDungeons.end()) {
            cq::send_group_message(GROUP_ID, bg_at(ev) + "无效的副本号!");
            return false;
        }

        // 检查玩家是否还有体力
        if (PLAYER.get_energy() < 10) {
            cq::send_group_message(GROUP_ID, bg_at(ev) + "你的体力已经耗尽了!");
            return false;
        }
        ...
        if (timeDiff < PLAYER.get_cd()) {
            cq::send_group_message(GROUP_ID, bg_at(ev) + "挑战冷却还没结束, 还剩" + std::to_string(timeDiff / 60) + ":" + std::to_string(timeDiff % 60));
            return false;
        }
    }
    catch (const std::exception &e) {
        cq::send_group_message(GROUP_ID, bg_at(ev) + "挑战前检查时出现错误! 请检查输入的副本号是否有效? 错误原因: " + e.what());
        return false;
    }
    catch (...) {
        cq::send_group_message(GROUP_ID, bg_at(ev) + "挑战前检查时出现错误! 请检查输入的副本号是否有效?");
        return false;
    }
    return true;
}
