...
static void dns_cb(struct mg_connection *c, int ev, void *ev_data,
  ...
  if (ev == MG_EV_POLL) {
    ...
    for (d = s_reqs; d != NULL; d = tmp) {
      tmp = d->next;
      // LOG(LL_DEBUG, ("%lu %lu dns po", d->expire, now));
...
#define MIME_ENTRY(_ext, _type) \
  ...
  const struct {
    ...
    const char *type;
             ...
             MIME_ENTRY("htm", "text/html; charset=utf-8"),
             MIME_ENTRY("shtm", "text/html; charset=utf-8"),
...
void mg_http_serve_dir(struct mg_connection *c, struct mg_http_message *hm,
  ...
  if (realpath(opts->root_dir, t1) == NULL) {
    ...
    if (mg_is_dir(t2)) {
      strncat(t2, "/index.htm", sizeof(t2) - strlen(t2) - 1);
      t2[sizeof(t2) - 1] = '\0';
      is_index = true;
    }
    ...
    if (strlen(t2) < n1 || memcmp(t1, t2, n1) != 0) {
      ...
      if (is_index && fp == NULL) {
        char *p = t2 + strlen(t2);
        while (p > t2 && p[-1] != '/') p--;
        strncpy(p, "index.shtm", &t2[sizeof(t2)] - p - 2);
        t2[sizeof(t2) - 1] = '\0';
        fp = mg_fopen(t2, "r");
      }
#endif
...
int mg_tls_init(struct mg_connection *c, struct mg_tls_opts *opts) {
  ...
  LOG(LL_DEBUG, ("%lu Setting TLS, CA: %s, cert: %s, key: %s", c->id, ca,
                 opts->cert == NULL ? "nu" : opts->cert,
                 opts->certkey == NULL ? "nu" : opts->certkey));
